class PushPull {
  constructor (plugin, conn) {
    ...
    this.ns = this.conn.name
    this.pusher = null
    this.puller = null
  }

  init = async () => {
    if (this.conn.options.pusherAutoStart) await this.startPusher()
  ...
  startPusher = async () => {
    ...
    this.pusher = sock
    this.plugin.log.debug('pusherStarted%s%s%d', this.ns, host, port)
  }
  ...
  startPuller = () => {
    ...
    this.puller = sock
    this.plugin.log.debug('pullerStarted%s%s%d', this.ns, host, port)
  }
}
